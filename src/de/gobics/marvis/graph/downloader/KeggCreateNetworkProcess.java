...
public class KeggCreateNetworkProcess extends AbstractNetworkCreator {
	...
	public MetabolicNetwork performTask() throws Exception {
		...
		sendDescription("Fetching information about the objects");

		LinkedList<InfoFetcher> jobs = new LinkedList<InfoFetcher>();
		setProgress(0);
		LinkedList<GraphObject> objects = graph.getAllObjects();
		setProgressMax(objects.size());
		for (GraphObject go : graph.getAllObjects()) {
			jobs.add(new InfoFetcher(this, go));
		}
		...
		try {
			...
			logger.info("Fetching process aborted");
	...
	private void buildRelations(MetabolicNetwork graph) throws MalformedURLException, IOException {
		...
		// Genes
		sendDescription("Fetching gene list for '" + organism + "' from KEGG");
		for (String gid : api.list_genes_of_organism(organism)) {
			graph.createGene(gid);
		}

		// Enzymes (bi-directional)
		sendDescription("Fetching enzymes list for '" + organism + "' from KEGG");
		...
		// Reactions
		sendDescription("Fetching Reactions of '" + organism + "'");
		...
		// Compounds
		sendDescription("Fetching compounds of '" + organism + "'");
		...
		sendDescription("Iterating over reactions to identify compounds");
		for (Reaction r : graph.getReactions()) {
			incrementProgress();
	...
	private class InfoFetcher implements Callable<Void> {
		...
		private void expand(Compound compound) throws Exception {
			...
			else if (info.get("MOL_WEIGHT") != null) {
		...
		private void expand(Reaction reaction) throws Exception {
			...
			for (int j = 0; j < tokens.length; j++) {
				...
				else if (tokens[j].matches("C\\d\\d\\d\\d\\d")) {
		...
		private void expand(Pathway pathway) throws Exception {
			KeggEntry info = api.get(pathway.getId());
			if (info.getId() == null) {
				logger.warning("Can not fetch info for: " + pathway.getId());
				return;
			}
